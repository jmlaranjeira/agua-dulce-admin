export const labels = {
  app: {
    name: 'Agua Dulce',
  },
  auth: {
    login: 'Iniciar sesion',
    logout: 'Cerrar sesion',
    register: 'Registrarse',
    email: 'Correo electronico',
    password: 'Contrasena',
    confirmPassword: 'Confirmar contrasena',
    name: 'Nombre',
    invalidCredentials: 'Credenciales incorrectas',
    userCreated: 'Usuario creado exitosamente',
    registrationDisabled: 'El registro esta deshabilitado. Ya existe un usuario administrador.',
    createFirstUser: 'Crear usuario administrador',
    goToLogin: 'Ir a iniciar sesion',
    passwordMismatch: 'Las contrasenas no coinciden',
  },
  menu: {
    dashboard: 'Dashboard',
    suppliers: 'Proveedores',
    products: 'Productos',
    stock: 'Stock',
    import: 'Importar',
    invoices: 'Facturas',
    customers: 'Clientes',
    orders: 'Pedidos',
    shippingZones: 'Zonas de envío',
    // Secciones
    sectionSales: 'VENTAS',
    sectionCatalog: 'CATÁLOGO',
    sectionPurchases: 'COMPRAS',
    sectionConfig: 'CONFIGURACIÓN',
  },
  pages: {
    dashboard: 'Dashboard',
    suppliers: 'Proveedores',
    supplierNew: 'Nuevo Proveedor',
    supplierEdit: 'Editar Proveedor',
    products: 'Productos',
    productNew: 'Nuevo Producto',
    productEdit: 'Editar Producto',
    stockMovements: 'Movimientos de Stock',
    import: 'Importar Productos',
    supplierOrders: 'Facturas de Proveedor',
    supplierOrderDetail: 'Detalle de Factura',
    customers: 'Clientes',
    customerNew: 'Nuevo Cliente',
    customerEdit: 'Editar Cliente',
    orders: 'Pedidos',
    orderNew: 'Nuevo Pedido',
    orderDetail: 'Detalle del Pedido',
    shippingZones: 'Zonas de Envío',
  },
  actions: {
    save: 'Guardar',
    cancel: 'Cancelar',
    delete: 'Eliminar',
    deactivate: 'Deshabilitar',
    edit: 'Editar',
    create: 'Crear',
    back: 'Volver',
    search: 'Buscar',
    filter: 'Filtrar',
    clear: 'Limpiar',
    actions: 'Acciones',
    activate: 'Activar',
    changeCategory: 'Cambiar categoría',
    changeSupplier: 'Cambiar proveedor',
    showInStore: 'Mostrar en tienda',
    hideFromStore: 'Ocultar de tienda',
  },
  status: {
    PENDING: 'Pendiente',
    PAID: 'Pagado',
    SHIPPED: 'Enviado',
    DELIVERED: 'Entregado',
    CANCELLED: 'Cancelado',
  },
  customerType: {
    RETAIL: 'Minorista',
    WHOLESALE: 'Mayorista',
    label: 'Tipo de cliente',
    wholesaleIndicator: 'Precio mayorista',
  },
  messages: {
    confirmDelete: '¿Estás seguro de que quieres eliminar este elemento?',
    savedSuccess: 'Guardado correctamente',
    deleteSuccess: 'Eliminado correctamente',
    errorGeneric: 'Ha ocurrido un error',
    noResults: 'No se encontraron resultados',
    loading: 'Cargando...',
    required: 'Este campo es obligatorio',
    invalidUrl: 'URL no válida',
  },
  suppliers: {
    newSupplier: 'Nuevo proveedor',
    noSuppliers: 'No hay proveedores registrados',
    confirmDelete: '¿Estás seguro de que quieres eliminar este proveedor?',
    deleteWarning: 'Esta acción no se puede deshacer.',
    savedSuccess: 'Proveedor guardado correctamente',
    deletedSuccess: 'Proveedor eliminado correctamente',
    supplierData: 'Datos del proveedor',
    allSuppliers: 'Todos los proveedores',
  },
  customers: {
    newCustomer: 'Nuevo cliente',
    noCustomers: 'No hay clientes registrados',
    savedSuccess: 'Cliente guardado correctamente',
    customerData: 'Datos del cliente',
    ordersSection: 'Pedidos del cliente',
    noOrders: 'Este cliente no tiene pedidos',
  },
  address: {
    title: 'Direcciones',
    add: 'Anadir direccion',
    edit: 'Editar direccion',
    label: 'Etiqueta',
    labelPlaceholder: 'Casa, Trabajo, etc.',
    street: 'Calle y numero',
    city: 'Ciudad',
    postalCode: 'Codigo postal',
    province: 'Provincia',
    country: 'Pais',
    notes: 'Notas de entrega',
    notesPlaceholder: 'Portero, instrucciones...',
    isDefault: 'Direccion principal',
    setDefault: 'Establecer como principal',
    confirmDelete: 'Eliminar esta direccion?',
    noAddresses: 'No hay direcciones registradas',
    addAddressHint: 'Puedes añadir direcciones desde el perfil del cliente',
    shippingAddress: 'Direccion de envio',
    selectAddress: 'Seleccionar direccion',
    noShippingAddress: 'Sin direccion de envio',
    savedSuccess: 'Direccion guardada correctamente',
    deletedSuccess: 'Direccion eliminada correctamente',
    defaultUpdated: 'Direccion principal actualizada',
  },
  products: {
    newProduct: 'Nuevo producto',
    noProducts: 'No hay productos registrados',
    savedSuccess: 'Producto guardado correctamente',
    deactivatedSuccess: 'Producto desactivado correctamente',
    deletedSuccess: 'Producto eliminado correctamente',
    confirmDelete: '¿Estás seguro de que quieres eliminar este producto?',
    deleteWarning: 'Esta acción no se puede deshacer.',
    cannotDeleteInOrder: 'No se puede eliminar un producto que pertenece a un pedido.',
    active: 'Activo',
    inactive: 'Inactivo',
    allStatus: 'Todos los estados',
    allSuppliers: 'Todos los proveedores',
    allCategories: 'Todas las categorías',
    margin: 'Margen',
    noSupplier: 'Sin proveedor',
    noCategory: 'Sin categoría',
    selectCategory: 'Seleccionar categoría',
    selectSupplier: 'Seleccionar proveedor',
    imageUrl: 'URL de imagen',
    // Bulk actions
    activatedSuccess: 'Productos activados correctamente',
    bulkDeactivatedSuccess: 'Productos desactivados correctamente',
    categoryChangedSuccess: 'Categoría actualizada correctamente',
    supplierChangedSuccess: 'Proveedor actualizado correctamente',
    shownSuccess: 'Productos mostrados en tienda',
    hiddenSuccess: 'Productos ocultados de tienda',
    // Visibility
    visible: 'Visible',
    hidden: 'Oculto',
    visibleInStore: 'Visible en tienda',
    visibleHint: 'Desactiva para productos que solo vendes por WhatsApp',
    allVisibility: 'Toda visibilidad',
  },
  orders: {
    newOrder: 'Nuevo pedido',
    noOrders: 'No hay pedidos registrados',
    createdSuccess: 'Pedido creado correctamente',
    statusUpdated: 'Estado actualizado correctamente',
    allStatus: 'Todos los estados',
    allCustomers: 'Todos los clientes',
    orderNumber: 'Nº Pedido',
    selectCustomer: 'Seleccionar cliente',
    selectProduct: 'Buscar producto...',
    addProduct: 'Añadir',
    removeItem: 'Quitar',
    subtotal: 'Subtotal',
    summary: 'Resumen',
    products: 'Productos',
    units: 'Unidades',
    margin: 'Margen',
    shipping: 'Envío',
    shippingFree: 'Gratis',
    shippingConsult: 'Consultar',
    emptyCart: 'Añade productos al pedido',
    minOneItem: 'Añade al menos un producto',
    markAsPaid: 'Marcar pagado',
    markAsShipped: 'Marcar enviado',
    markAsDelivered: 'Marcar entregado',
    cancelOrder: 'Cancelar pedido',
    confirmCancel: '¿Estás seguro de que quieres cancelar este pedido?',
    customerInfo: 'Datos del cliente',
    orderItems: 'Productos del pedido',
    sendWhatsApp: 'WhatsApp',
    copyMessage: 'Copiar',
    messageCopied: 'Mensaje copiado al portapapeles',
    downloadPdf: 'PDF',
  },
  whatsapp: {
    greeting: 'Hola! Te envío el resumen de tu pedido:',
    greetingPlain: 'Hola! Te envio el resumen de tu pedido:',
    paymentMethods: 'Formas de pago:',
    bizum: 'Bizum: {phone}',
    revolut: 'Revolut: {tag}',
    iban: 'Transferencia: {iban}',
    thanks: '¡Gracias! 💙',
    thanksPlain: 'Gracias!',
  },
  fields: {
    name: 'Nombre',
    phone: 'Teléfono',
    email: 'Email',
    notes: 'Notas',
    code: 'Código',
    price: 'Precio',
    priceRetail: 'Precio venta',
    priceWholesale: 'Precio mayorista',
    costPrice: 'Precio coste',
    supplier: 'Proveedor',
    category: 'Categoría',
    customer: 'Cliente',
    type: 'Tipo',
    status: 'Estado',
    date: 'Fecha',
    quantity: 'Cantidad',
    total: 'Total',
    url: 'URL',
    actions: 'Acciones',
    stock: 'Stock',
    unitCost: 'Coste unitario',
    totalCost: 'Coste total',
    reference: 'Referencia',
  },
  import: {
    // Steps
    stepSource: 'Origen',
    stepSearch: 'Buscar',
    stepConfigure: 'Configurar',
    stepConfirm: 'Confirmar',

    // Step 1
    selectSource: 'Selecciona el origen de importación',
    sourceQuestion: '¿De dónde quieres importar?',
    sourceSubtitle: 'Selecciona el origen de los productos que deseas añadir',
    comingSoon: 'Próximamente',

    // Source metadata
    sources: {
      'rainbow-silver': {
        name: 'Rainbow Silver',
        description: 'Catálogo web con precios actualizados',
      },
      'rainbow-invoice': {
        name: 'Rainbow Factura',
        description: 'Importar desde PDF de factura',
      },
      'panbubu-email': {
        name: 'Panbubu Email',
        description: 'Extraer productos desde email (.eml)',
      },
      'excel-supplier': {
        name: 'Excel de Proveedor',
        description: 'Subir archivo Excel con productos',
      },
      'mayorista-plata': {
        name: 'Mayorista Plata',
        description: 'Importar desde PDF de factura',
      },
    } as Record<string, { name: string; description: string }>,

    // Step 2
    searchText: 'Buscar productos',
    searchPlaceholder: 'Nombre, código, tags...',
    filterByCategory: 'Filtrar por categoría',
    allCategories: 'Todas las categorías',
    productType: 'Tipo de producto',
    selectType: 'Seleccionar tipo',
    targetSupplier: 'Proveedor destino',
    defaultCategory: 'Categoría por defecto',
    searchProducts: 'Buscar',

    // Step 3
    selectProducts: 'Selecciona y configura los productos',
    image: 'Imagen',
    cost: 'Coste',
    retail: 'PVP',
    b2b: 'Mayorista',
    statusNew: 'Nuevo',
    statusExists: 'Ya existe',
    statusAddStock: '+ Stock',
    statusDuplicate: 'Duplicado',
    codeExistsInDb: 'Codigo ya existe',
    codeDuplicateInBatch: 'Codigo duplicado en lote',
    newProducts: 'nuevos',
    stockAddition: 'añadir stock',
    applyMargin: 'Aplicar margen',
    customMargin: 'Margen personalizado',
    selected: 'seleccionados',
    loadMore: 'Cargar más',

    // Step 4
    summary: 'Resumen de importación',
    productsToImport: 'Productos a importar',
    productsSkipped: 'Productos omitidos (ya existen)',
    targetSupplierLabel: 'Proveedor destino',
    categoryLabel: 'Categoría',
    importProducts: 'Importar productos',

    // Messages
    importSuccess: 'Importación completada',
    importedCount: '{imported} productos importados, {skipped} omitidos',
    noProductsSelected: 'Selecciona al menos un producto',
    missingRetailPrice: 'Todos los productos seleccionados deben tener precio retail',

    // Navigation
    next: 'Siguiente',
    previous: 'Anterior',

    // Invoice import
    uploadInvoice: 'Subir factura PDF',
    dropInvoice: 'Arrastra el PDF aquí o haz clic para seleccionar',
    invoiceEurOnly: 'Solo facturas en EUR (no BAHT)',
    foundInApi: 'API',
    notFoundInApi: 'PDF',
    invoiceSummary: '{total} productos, {found} encontrados en API, {existing} ya existen',
    parsingInvoice: 'Procesando factura...',
    invoiceExists: 'Factura ya registrada',
    invoiceExistsDetail: 'La factura {number} ya está en el sistema. Puedes continuar pero no se creará un duplicado.',
    viewExistingInvoice: 'Ver factura existente',
    totalStockToAdd: 'Stock total a añadir',

    // Email import
    parsingEmail: 'Procesando email...',
    uploadEmail: 'Subir Email (.eml)',
    emailHint: 'Sube el email de confirmación de Panbubu',
    emailSummary: '{total} productos encontrados, {existing} ya existen',
    orderExists: 'Este pedido ya existe',
    orderExistsDetail: 'El pedido {number} ya fue importado anteriormente',
    viewExistingOrder: 'Ver pedido existente',
  },
  stock: {
    stock: 'Stock',
    lowStock: 'Stock bajo',
    lowStockWarning: 'Quedan pocas unidades',
    movements: 'Movimientos',
    noMovements: 'No hay movimientos registrados',
    allTypes: 'Todos los tipos',
    allProducts: 'Todos los productos',
    PURCHASE: 'Compra',
    SALE: 'Venta',
    ADJUSTMENT: 'Ajuste',
    RETURN: 'Devolución',
    adjustStock: 'Ajustar Stock',
    adjustmentQuantity: 'Cantidad a ajustar',
    adjustmentNotes: 'Motivo del ajuste',
    positiveInfo: 'Positivo = entrada, Negativo = salida',
    currentStock: 'Stock actual',
    newStock: 'Stock resultante',
    adjustSuccess: 'Stock ajustado correctamente',
    viewHistory: 'Ver historial',
    insufficientStock: 'El stock no puede quedar negativo',
  },
  supplierOrders: {
    noOrders: 'No hay facturas registradas',
    invoiceNumber: 'Nº Factura',
    invoiceDate: 'Fecha factura',
    itemCount: 'Productos',
    totalAmount: 'Importe total',
    subtotalProducts: 'Subtotal productos',
    shipping: 'Envío',
    grandTotal: 'Total factura',
    allSuppliers: 'Todos los proveedores',
    viewPdf: 'Ver PDF',
    downloadPdf: 'Descargar factura',
    confirmDelete: '¿Eliminar esta factura? Se revertirá el stock.',
    deletedSuccess: 'Factura eliminada correctamente',
    orderItems: 'Productos de la factura',
  },
  shipping: {
    noZones: 'No hay zonas de envío configuradas',
    zoneName: 'Zona',
    price: 'Precio',
    freeAbove: 'Gratis desde',
    transitDays: 'Tiempo tránsito',
    days: 'días',
    customs: 'Aduanas',
    active: 'Activa',
    yes: 'Sí',
    no: 'No',
    editZone: 'Editar zona de envío',
    transitDaysMin: 'Días mínimos',
    transitDaysMax: 'Días máximos',
    hasCustomsRisk: 'Riesgo aduanas',
    customsHint: 'Mostrar aviso de posibles trámites aduaneros',
    never: 'Nunca gratis',
    freeAboveHint: 'Dejar vacío si nunca es gratis',
    savedSuccess: 'Zona de envío guardada correctamente',
    // Supplier form
    deliveryTimes: 'Tiempos de entrega',
    deliveryDaysMin: 'Días mínimos',
    deliveryDaysMax: 'Días máximos',
    deliveryDaysMinHint: 'Tiempo mínimo de entrega del proveedor',
    deliveryDaysMaxHint: 'Tiempo máximo de entrega del proveedor',
    isInternational: 'Proveedor internacional',
    isInternationalHint: 'Si está activo, los pedidos mostrarán aviso de posibles aduanas',
    // Order detail
    shippingInfo: 'Información de envío',
    zone: 'Zona',
    shippingCost: 'Coste envío',
    free: 'Gratis',
    estimatedTime: 'Tiempo estimado',
    consult: 'Consultar',
    customsWarning: 'Posible trámite de aduanas',
    noZone: 'No especificada',
    needsSupplierOrder: 'Requiere pedido a proveedor',
  },
}
